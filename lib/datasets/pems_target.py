...
class Pems04(PandasDataset):
    ...
    def load(self, impute_zeros=True):
        data = np.load('./datasets/PEMS/pems04.npz')
        data = data['data']
    ...
    def load_distance_matrix(self, ids):
        ...
        try:
            ...
            distances = pd.read_csv('./datasets/PEMS/distance_4.csv')
            num_sensors = len(ids)
...
class MissingValuesPems04(Pems04):
    SEED = 56789

    def __init__(self, p_fault=0.0015, p_noise=0.05, fixed_mask=False):
        super(MissingValuesPems04, self).__init__()
        ...
        # print(eval_mask_block.size)
       
        # print(eval_mask_block.size - np.count_nonzero(eval_mask_block))
        # print(self.df.shape)
        # exit()
    ...
    def training_mask(self):
        mask = self.mask if self.eval_mask is None else (self.mask & (1 - self.eval_mask))
