...
def sample_mask_block(shape, p=0.002, p_noise=0., max_seq=1, min_seq=1, rng=None):
    if rng is None:
...
def sample_mask(data,p=0.02):
    mask = data != 0
    # print(int(mask.size*p)-np.count_nonzero(mask == 0))
    # print(np.count_nonzero(mask == 0))
    # print(np.count_nonzero(mask == 1))
    true_indices = np.argwhere(mask)
    if int(mask.size*p) > np.count_nonzero(mask == 0):
        num_to_convert = int(mask.size*p)-np.count_nonzero(mask == 0)
    else:
        num_to_convert = 0
    # Randomly shuffle the indices
    np.random.shuffle(true_indices)

    # Select and convert 25% of True values to False
    for i in range(num_to_convert):
        row, col = true_indices[i]
        mask[row, col] = False
    return mask.astype('uint8')
