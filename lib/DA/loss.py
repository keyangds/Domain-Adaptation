...
def Coral(source, target): 
    ...
    target = target.reshape(target.shape[0]*target.shape[2], target.shape[1])
...
class VAT(nn.Module):
    ...
    def forward(self, X, mask, logit):
        vat_loss = self.virtual_adversarial_loss(X, mask, logit)
        return vat_loss

    def generate_virtual_adversarial_perturbation(self, x, mask, logit):
        d = torch.randn_like(x, device=self.device)

        for _ in range(self.n_power):
            d = self.XI * self.get_normalized_vector(d).requires_grad_()
            logit_m, _ = self.model(x + d, mask)
    ...
    def virtual_adversarial_loss(self, x, mask, logit):
        r_vadv = self.generate_virtual_adversarial_perturbation(x, mask, logit)
        logit_p = logit.detach()
        logit_m, _ = self.model(x + r_vadv, mask)
