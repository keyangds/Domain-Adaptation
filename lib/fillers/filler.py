...
class Filler(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx=0):
        ...
        if self.da_method != None:
            ...
            if self.da_method == 'direct':
                ...
                loss_da = Coral(src_feat, trg_feat)
                loss += loss_da
            elif self.da_method == 'cotmix':
                h = self.temporal_shift // 2
                ## source dominant mix
                src_xm, trg_xm = self.temporal_mixup(src_x, trg_x, self.mix_ratio, h)
                src_ymh, src_ymh_r = self.model(src_xm, src_mask)
                trg_ymh, trg_ymh_r = self.model(trg_xm, trg_mask)

                ntx = NTXentLoss(self.device, batch_size=trg_feat.shape[0], temperature=0.3, use_cosine_similarity = True)
                source_ntx = ntx.forward(src_feat.view(src_feat.shape[0],-1), src_ymh_r.view(src_ymh_r.shape[0],-1))
                target_ntx = ntx.forward(trg_feat.view(trg_feat.shape[0],-1), trg_ymh_r.view(trg_ymh_r.shape[0],-1))
                ...
                domain_label_src = torch.ones(src_feat.size(0), dtype=torch.long)
                domain_label_trg = torch.zeros(trg_feat.size(0), dtype=torch.long)
                ...
                src_domain_loss = criterion(src_domain_pred, domain_label_src.long())
            
                trg_domain_pred = self.domain_classifier(trg_feat_reversed)
                trg_domain_loss = criterion(trg_domain_pred, domain_label_trg.long())
                ...
                feature_concat = torch.cat((src_feat, trg_feat), dim=0)
                imputation_concat = torch.cat((src_pred, trg_pred), dim=0)

                domain_loss = 0
                self.cross_entropy = nn.CrossEntropyLoss()

                feat_x_pred = torch.bmm(feature_concat, imputation_concat.transpose(1, 2)).detach()
                disc_prediction = self.domain_classifier(feat_x_pred.view(-1, feat_x_pred.size(1) * feat_x_pred.size(2)))
           
                domain_loss = self.cross_entropy(disc_prediction, domain_labels)
                ...
                ## Feature
                feat_concat = torch.cat((src_feat, trg_feat), 0)
                ...
                dis_pred = self.discriminator.forward(feat_concat.detach())
                dis_loss = self.cross_entropy(dis_pred, domain_labels)
                ...
                dis_pred = self.discriminator.forward(feat_concat)
                domain_loss = self.cross_entropy(dis_pred, domain_labels)
                ...
                src_feat, out_s = self.feature_extractor.forward(src_x)   
                trg_feat, out_t = self.feature_extractor.forward(trg_x)
                print(out_t.shape)
                print(trg_feat.shape)
                exit()
    ...
    def validation_step(self, batch, dataloader_idx = 1):
        ...
        trg_pred, _ = self.model(trg_x, trg_mask)
